...
namespace Organilog.Services
{
    public class SyncInvoiceService : BaseService, ISyncInvoiceService
    {
        ...
        {
            Settings.LastSyncInvoice = ((long)DateTime.Now.AddHours(0).ToUnixTimestamp()).ToString();

            if (App.LocalDb.Table<Setting>().ToList().Find(se => se.Name.Equals("APP_LAST_SYNCHRO_INVOICE")) is Setting lastSyncInvoice)
            {
                lastSyncInvoice.Value = Settings.LastSyncInvoice;
        ...
        private void InsertOrUpdateInvoice(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateInvoiceProduct(Dictionary<string, object> data)
        {
            try
            {
