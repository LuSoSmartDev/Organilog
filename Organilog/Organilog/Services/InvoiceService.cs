...
namespace Organilog.Services
{
    public class InvoiceService : BaseService, IInvoiceService
    {
        ...
        {
            IEnumerable<Invoice> result = App.LocalDb.Table<Invoice>().ToList().FindAll(inv => inv.UserId == CurrentUser.Id && inv.IsInvoice != 1 && inv.IsInvoice != 2 && (inv.FkClientServerId > 0 || !inv.FkClientAppId.Equals(Guid.Empty)) && inv.IsActif == 1)
                .OrderByDescending(inv => inv.ServerId).ThenByDescending(inv => inv.AddDate).ThenByDescending(inv => inv.IDueDate).ThenByDescending(inv => inv.EditDate);
        ...
        {
            IEnumerable<Invoice> result = App.LocalDb.Table<Invoice>().ToList().FindAll(inv => inv.UserId == CurrentUser.Id && (inv.IsInvoice == 1 || inv.IsInvoice == 2) &&(inv.FkClientServerId>0 || !inv.FkClientAppId.Equals(Guid.Empty)) && inv.IsActif == 1)
                .OrderByDescending(iv=>iv.ServerId).ThenByDescending(inv=>inv.AddDate)
                .ThenByDescending(inv => inv.IDate).ThenByDescending(inv => inv.IDueDate).ThenByDescending(inv => inv.EditDate);
        ...
        private void GetRelations(Invoice invoice)
        {
            ...
            invoice.MediaLinks = App.LocalDb.Table<MediaLink>().ToList().FindAll(medl => medl.UserId == CurrentUser.Id && ((medl.FkColumnServerId > 0 && medl.FkColumnServerId == invoice.ServerId) || (!medl.FkColumnAppliId.Equals(Guid.Empty) && medl.FkColumnAppliId.Equals(invoice.Id))) && !medl.IsDelete && medl.IsActif == 1).ToObservableCollection();
            foreach (var medl in invoice.MediaLinks)
            {
