...
namespace Organilog.Services
{
    public class InterventionService : BaseService, IInterventionService
    {
        public Task<List<Intervention>> GetInterventions(DateTime SelectedDate, bool showNotDone, int limit = 0)
        {
            ...
            if (showNotDone)
                unfinished = assigned.FindAll(i => (i.IsDone == 0 || i.IsDone==2) && ((i.PlanningDateStart.HasValue && i.PlanningDateStart.Value.Date <= SelectedDate.Date)
                                                                    || (!i.PlanningDateStart.HasValue && !i.PlanningDateStart.HasValue &&
                                                                    ((i.EditDate.HasValue && i.EditDate.Value.Date <= SelectedDate.Date)
                                                                    ||(i.AddDate.HasValue && i.AddDate.Value.Date <= SelectedDate.Date)))))
                                                                    .OrderByDescending(i => i.EditDate).ThenBy(i => i.DoneDateStart).ThenBy(i => i.PlanningDateStart ).ThenBy(i=>i.PlanningHourStart).ThenBy(i => i.DoneHourStart);
                                                                    //.OrderByDescending(i => i.PlanningDateStart).ThenBy(i => i.AddDate).ThenBy(i => i.EditDate).ThenBy(i => i.PlanningHourStart).ThenBy(i => i.PlanningHourEnd);
            else
                unfinished = assigned.FindAll(i => (i.IsDone == 0 || i.IsDone == 2) && ((i.PlanningDateStart.HasValue && i.PlanningDateStart.Value.Date == SelectedDate.Date)
                                                                    || (!i.PlanningDateStart.HasValue && i.DoneDateStart.HasValue && i.DoneDateStart.Value.Date == SelectedDate.Date)
                                                                    || (!i.PlanningDateStart.HasValue && i.AddDate.HasValue && i.AddDate.Value.Date == SelectedDate.Date)
                                                                    || (!i.PlanningDateStart.HasValue && i.EditDate.HasValue && i.EditDate.Value.Date == SelectedDate.Date)))
                                                                    .OrderBy(i => i.PlanningDateStart).ThenBy(i => i.PlanningHourStart).ThenBy(i=>i.DoneDateStart).ThenBy(i=>i.DoneHourStart).ThenBy(i => i.EditDate);
            ...
            var finished = assigned.FindAll(i => i.IsDone == 1 && ((i.DoneDateStart.HasValue && i.DoneDateEnd.HasValue && i.DoneDateStart.Value.Date <= SelectedDate.Date && SelectedDate.Date <= i.DoneDateEnd.Value.Date)
                                                                    || (i.DoneDateStart.HasValue && i.DoneDateStart.Value.Date == SelectedDate.Date) 
                                                                    || (i.DoneDateEnd.HasValue && i.DoneDateEnd.Value.Date == SelectedDate.Date)))
            ...
            unfinished = unfinished.ToList().FindAll(e => e.PlanningDateStart == null || (e.PlanningDateStart.HasValue && e.PlanningDateStart.Value.Date <= SelectedDate.Date));
            ...
            result = result.OrderBy(i=>i.PlanningDateStart).ThenBy(i=> convertTimeToInt(i.PlanningHourStart)).ThenBy(i => i.DoneDateStart).ThenBy(i=>convertTimeToInt(i.DoneHourStart)).ThenByDescending(i=>i.EditDate);
        ...
        public Task<List<Intervention>> GetNotAssignedInterventions(DateTime? SelectedDate = null, int offset = 0, int limit = 0)
        {
            ...
            if (SelectedDate.HasValue && AppSettings.MobileShowNumberCalendarOnNonAssigned)
            {
                result = App.LocalDb.Table<Intervention>().ToList().FindAll(i => i.FkUserServerlId == 0 && i.FkUserAppId.Equals(Guid.Empty)
                                                                            ...
                                                                            && i.IsActif == 1)
                                                                            .OrderByDescending(i => i.PlanningDateStart).ThenBy(i => i.PlanningHourStart);
            }
            else
            {
                result = App.LocalDb.Table<Intervention>().ToList().FindAll(i =>  (i.FkUserAppId.Equals(Guid.Empty) && i.FkUserServerlId == 0) && (!i.FkClientAppId.Equals(Guid.Empty) || i.FkClientServerId != 0) && i.IsActif == 1)
                                                                            .OrderByDescending(i=>i.EditDate).ThenByDescending(i=>i.AddDate);
            }
        ...
        private void GetClientAddress(Intervention intervention)
        {
            ...
            intervention.Category = App.LocalDb.Table<Category>().FirstOrDefault(group => group.UserId == CurrentUser.Id && (group.ServerId > 0 && group.ServerId == intervention.FkGroupServerId) && group.IsActif == 1);
        }
