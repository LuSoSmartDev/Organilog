...
using Organilog.AppResources.Localization;
...
namespace Organilog.Services
{
    public class SyncDataService : BaseService, ISyncDataService
    {
        ...
        private readonly ISyncIssueService syncIssueService;
        private readonly ISyncEquipmentService syncEquipmentService;
        ...
        public SyncDataService(ISyncProductService syncProductService, ISyncInvoiceService syncInvoiceService,ISyncIssueService syncIssueService,ISyncEquipmentService syncEquipment)
        {
            this.syncProductService = syncProductService;
            this.syncInvoiceService = syncInvoiceService;
            this.syncIssueService = syncIssueService;
            this.syncEquipmentService = syncEquipment;
        }
        ...
        public Task<bool> SyncFromServer(int method, Action onSuccess, Action<string> onError = null, bool showOverlay = false)
        {
            ...
            Task.Run(async () =>
            {
                return (await syncProductService.SyncFromServer(method) && await syncInvoiceService.SyncFromServer(method) && await syncIssueService.SyncFromServer(method) && await syncEquipmentService.SyncFromServer(method) &&  await SyncFromServer(method) );
        ...
        public async Task<bool> SyncFromServer(int method)
        {
            ...
            if (!Settings.LastSync.Equals("0") && !Settings.LastSync.Equals(""))
            {
                ...
                //var ts = long.Parse(Settings.LastSync);
               // DateTime start = new DateTime(1970, 1, 1, 0, 0, 0).AddMilliseconds(ts);
                

               // Settings.LastSync = start.AddHours(-2).AddMinutes(-10).ToUnixTimestamp().ToString();
            }

            string url = ApiURI.URL_BASE(CurrentAccount) + ApiURI.URL_GET_SYNC(CurrentUserName, Settings.CurrentPassword, Settings.LastSync);
        ...
        {
            Settings.LastSync = ((long)DateTime.Now.AddHours(0).ToUnixTimestamp()).ToString();
                        ...
                        if (item != null && item.V != null)
                        {
                            switch (item.I)
                            {
                                ...
                                case "con": //contract
                                    ...
                                    break;
                                case "intg"://list group
                                    InsertOrUpdateCategory(item.V);
                                    break;
                                default:
                                    break;
                            }
        ...
        private void InsertOrUpdateCategory(Dictionary<string, object> data)
        {
            var result = JsonConvert.DeserializeObject<CategoryResponse>(JsonConvert.SerializeObject(data), App.DefaultDeserializeSettings);
            if(result == null || result.ServerId == 0)
                return;
            

            if (App.LocalDb.Table<Category>().FirstOrDefault(group => group.ServerId == result.ServerId) is Category category)
            {
                //PropertyExtension.UpdateProperties(result, address, ExcludedUpdateProperties);
                category.UpdateFromResponse(result);

                category.UserId = CurrentUser.Id;

                App.LocalDb.Update(category);
                totalRowUpdated++;
            }
            else
            {
                App.LocalDb.InsertOrReplace(new Category(result)
                {
        ...
        private void InsertOrUpdateContract(Dictionary<string, object> data)
        {
        ...
        private void InsertOrUpdateAddress(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateClient(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateChemin(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateFiliale(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateIntervention(Dictionary<string, object> data)
        {
            try
            {
                ...
                if (App.LocalDb.Table<Intervention>().FirstOrDefault(adr => adr.ServerId == result.ServerId) is Intervention intervention)
                {
                    ...
                    intervention.UserId = result.FkUserServerlId;
                    if (result.FkUserServerlId == 0)
                    {
                        intervention.FkUserAppId = Guid.Empty;
                    }
                ...
                else
                {
                    ...
                    {
                        UserId = CurrentUser.Id,
                        AddDate = result.AddDate != null ? result.AddDate : new DateTime(),
        ...
        private void InsertOrUpdateLinkInterventionProduct(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateLinkInterventionTask(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateMessage(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateMedia(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateMediaLink(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateProduct(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateTasks(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateCategoryTracking(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateSetting(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateUser(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateUnite(Dictionary<string, object> data)
        {
            try
            {
                ...
                {
                    if(result.FilialeServerKey <= 0 )
                    {
                        var test = result.ServerId;
                    }
        ...
        private void InsertOrUpdateUniteItem(Dictionary<string, object> data)
        {
            try
            {
        ...
        private void InsertOrUpdateUniteLink(Dictionary<string, object> data)
        {
            try
            {
        ...
        public async Task<bool> SyncToServer(int method)
        {
            ...
            var unite_links = App.LocalDb.Table<UniteLink>().Where(unl => unl.UserId == CurrentUser.Id && unl.UniteValue != null && unl.IsToSync).ToArray(); // && !unl.UniteValue.Equals("0")
            var mediaLinks = App.LocalDb.Table<MediaLink>().Where(medl => medl.UserId == CurrentUser.Id && medl.IsToSync).ToArray();
            if(unite_links!=null && unite_links.Length > 0)
            {
                foreach(var uni in unite_links)
                {
                    var temp = uni.UniteValue;
                    var itemServerId = "";
                    if (temp.Contains(";"))
                    {
                        var splData = temp.Split(';');
                        int check = 0;
                        foreach(var item in splData) {
                            var isNumeric = int.TryParse(item, out int n);
                            if (isNumeric)
                            {
                                itemServerId = temp.Replace(';', ',');
                                break;
                            }
                            var temp1 = item.Trim();
                            var uniteValueItem = App.LocalDb.Table<UniteItem>().FirstOrDefault(e => e.Value == temp1);
                            if (uniteValueItem != null)
                            {
                                if (check > 0)
                                    itemServerId += ",";
                                itemServerId += uniteValueItem.ServerId;
                                check++;
                            }
                    ...
                    else
                    {
                        ...
                        var uniteValueItem = App.LocalDb.Table<UniteItem>().FirstOrDefault(e => e.Value == temp && uni.FkUniteServerId == e.UniteServerId);
            ...
            var url = ApiURI.URL_BASE(CurrentAccount) + ApiURI.URL_SET_SYNC(CurrentUserName, Settings.CurrentPassword, method);
        ...
        private void SyncMediaToServer(MediaLink mediaLink, Media media, int method)
        {
            Task.Run(async () =>
            {
                //string time1 = media.AddDate?.ToLongTimeString();
                string mediaInfo = "";
                ...
                mediaInfo += "&mCreatedOn=" +  ConvertToTimestamp( media.AddDate);
                mediaInfo += "&mModifDate=" + ConvertToTimestamp(media.EditDate);
                ...
                if(media.Type!=null && media.Type.Equals("invoice"))
                    mediaInfo += "&mType=invoice";
                if(media.Type!=null && media.Type.Equals("issue"))
                    mediaInfo += "&mType=issue";
        ...
        public void sendPushingAlertIntervention()
        {
            if (!AppSettings.ActiveNotifInterventionStart)
                return;

            //Debug.WriteLine("DKM sendPushingAlertIntervention");
            var interventions = App.LocalDb.Table<Intervention>().Where(it => it.UserId == CurrentUser.Id && it.IsActif == 1 &&
            it.PlanningDateStart != null && it.PlanningDateStart == DateTime.Today && it.IsDone == 0 && it.PlanningHourStart!=null ).ToArray();

            if (interventions.Length != 0)
            {
                for (int i = 0; i < interventions.Length; i++)
                {
                    var it = interventions[i].PlanningHourStart.Split(':');
                    if (it == null || it.Length < 2)
                        continue;

                    var date = DateTime.Now.AddMinutes(10);
                    var date1 = DateTime.Now.AddMinutes(9);
                    var src = DateTime.Now;
                    var hm = new DateTime(src.Year, src.Month, src.Day, int.Parse(it[0]), int.Parse(it[1]), 0);
                    Debug.WriteLine("hm : " + hm + " date : " + date + " date1 :"+ date1);
                    if(hm <= date && hm > date1)
                       // Debug.WriteLine("DKM sendPushingAlertIntervention showing ");
                    DependencyService.Get<ILocalNotificationService>().Show(new LocalNotification
                        {
                            NotificationId = 102,
                            Title = "Une intervention débute à " + interventions[i].PlanningHourStart,
                            Description = "L'intervention n°"+ interventions[i].Code + " commence dans 10 min."  ,
                        });
                }
            }

            //if (AppSettings.NewInterventionNotification)
            //{
            //    DependencyService.Get<ILocalNotificationService>().Show(new LocalNotification
            //    {
            //        NotificationId = 102,
            //        Title = "Une intervention débute à HH:MM",
            //        Description = totalInterventionInserted != 1 ? Resources.notification_new_interventions : Resources.notification_new_intervention,
