...
namespace Organilog
{
    public class RestClient : IRestClient
    {
        ...
        public async Task<HttpResponseMessage> GetAsync(string url, CancellationToken? token = null)
        {
            ...
            //day em lam sao them header vao ?
            ...
            if (token != null)
                using (HttpClient client = new HttpClient(new HttpClientHandler { UseCookies = false }))
                {
                    ...
                    client.DefaultRequestHeaders.Add("Cookie", "PHPSESSID=klq1t8ggrc1m8j300pmahvmuv2");
                    return await client.GetAsync(uri, token.Value);
                }
