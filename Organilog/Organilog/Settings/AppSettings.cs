...
namespace Organilog
{
    public class AppSettings
    {
        ...
        public static bool MobileShowNumberTitle { get; set; }
        public static bool MobileShowNumberCalendarOnNonAssigned { get; set; }
        // View Intervention
        ...
        public static bool ActiveNotifInterventionStart { get; set; }
        ...
        public static bool MobileVisibleWithFilialeZero { get; set; }
        ...
        public static void ReloadSetting()
        {
            ...
            Settings.LastSyncProduct = lastSyncProduct;

            var lastSyncInvoice = App.LocalDb.Table<Setting>().ToList().Find(se => se.Name.Equals("APP_LAST_SYNCHRO_INVOICE"))?.Value ?? "0";
            Settings.LastSyncInvoice = lastSyncInvoice;

            var lastSyncIssue = App.LocalDb.Table<Setting>().ToList().Find(se => se.Name.Equals("APP_LAST_SYNCHRO_ISSUE"))?.Value ?? "0";
            Settings.LastSyncIssue = lastSyncIssue;

            var lastSyncEquipment = App.LocalDb.Table<Setting>().ToList().Find(se => se.Name.Equals("APP_LAST_SYNCHRO_EQUIPMENT"))?.Value ?? "0";
            Settings.LastSyncEquipment = lastSyncEquipment;
            ...
            MobileShowNumberTitle = (App.LocalDb.Table<Setting>().ToList().Find(se => se.Name.Equals("MOBILE_DISPLAY_HOME_SHOW_NUMBER"))?.Value ?? "0") != "0";
            MobileShowNumberCalendarOnNonAssigned = (App.LocalDb.Table<Setting>().ToList().Find(se => se.Name.Equals("MOBILE_DISPLAY_HOME_NOT_ASSIGNED_WITHOUT_CALENDAR"))?.Value ?? "0") == "0";
            ...
            //show tasks with filiale = 0 
            MobileVisibleWithFilialeZero = (App.LocalDb.Table<Setting>().ToList().Find(se => se.Name.Equals("MOBILE_ACTIVITY_TYPE"))?.Value ?? "0") != "0";
            ...
            MobileHourStartPauseDefault = App.LocalDb.Table<Setting>().ToList().Find(se => se.Name.Equals("MOBILE_HOUR_START_PAUSE_DEFAULT"))?.Value ?? "12:00";
            ...
            ActiveNotifInterventionStart = (App.LocalDb.Table<Setting>().ToList().Find(se => se.Name.Equals("MOBILE_NOTIF_INTERVENTION_START"))?.Value ?? "0") != "0";
