...
namespace Organilog.ViewModels.SystemSettings
{
        ...
        private void Set_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            if (sender is Setting setting)
            {
                if (setting.Name.Equals("APP_RESET_DATA"))
                {
                    ...
                    Settings.LastSyncIssue = "0";
                    if (ListSetting.Find(se => se.Name.Equals("APP_LAST_SYNCHRO_ISSUE")) is Setting lastSyncIssue)
                    {
                        lastSyncIssue.Value = "0";
                        App.LocalDb.Update(lastSyncIssue);
                    }
                    Settings.LastSyncEquipment = "0";
                    if (ListSetting.Find(se => se.Name.Equals("APP_LAST_SYNCHRO_EQUIPMENT")) is Setting lastSyncEquipmnet)
                    {
                        lastSyncEquipmnet.Value = "0";
                        App.LocalDb.Update(lastSyncEquipmnet);
                    }
                    ClearAllData();
