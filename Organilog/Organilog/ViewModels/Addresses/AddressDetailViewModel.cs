...
using Organilog.ViewModels.Customers;
...
namespace Organilog.ViewModels.Addresses
{
    public class AddressDetailViewModel : TinyViewModel
    {
        ...
        private async void OpenMap(object sender, TaskCompletionSource<bool> tcs)
        {
            ...
            //{
               // DependencyService.Get<IOpenMapIntentService>().Open(Address.Latitude, Address.Longitude);

                 var location = new  Xamarin.Essentials.Location(Address.Latitude, Address.Latitude);
            ...
            //    Device.OpenUri(new Uri(string.Format("http://maps.apple.com/?daddr=" + Address.Latitude + "," + Address.Longitude + "")));
        ...
        private async void AddIntervention(Client value, TaskCompletionSource<bool> tcs)
        {
            ...
            {
                { ContentKey.SELECTED_CUSTOMER, value},
                { ContentKey.SELECTED_ADDRESS, Address}
            };
