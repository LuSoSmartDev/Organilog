    ...
    public class ProductLookUpViewModel : TinyViewModel
    {
        ...
        private void OnsearchKeyChanged()
        {
            ...
            Task.Run(() =>
            {
                ...
                {
                    return App.LocalDb.Table<Product>().ToList().FindAll(pr => pr.UserId == CurrentUserId  && ((!string.IsNullOrWhiteSpace(pr.Code) && pr.Code.Contains(searchKey)) || (!string.IsNullOrWhiteSpace(pr.Nom) && pr.Nom.UnSignContains(SearchKey))) && pr.IsActif == 1).Take(20);
                }
