...
namespace Organilog.ViewModels.Tracking
{
    public class TrackingViewModel : TinyViewModel
    {
        ...
        private bool isLocationEnable = true;

        public ICommand DeleteTracking { get; private set; }
        ...
        public TrackingViewModel(ISyncDataService syncService)
        {
            ...
            DeleteTracking = new AwaitCommand<Models.Tracking>(DeleteTrackingCommand);

        }

        private void DeleteTrackingCommand(Models.Tracking tracking, TaskCompletionSource<bool> tcs)
        {
            App.LocalDb.Delete(tracking);
            GetListTracking();
        ...
        public override async void InitAsync(NavigationParameters parameters)
        {
            ...
            ListCategoryTracking = App.LocalDb.Table<CategoryTracking>().ToList().FindAll(ct => ct.UserId == CurrentUserId && ct.IsActif == 1).OrderBy(e=>e.Sort).ToList();

            GetListTracking();
            if (await LocationHelper.CheckLocationPermission(true))
            {
