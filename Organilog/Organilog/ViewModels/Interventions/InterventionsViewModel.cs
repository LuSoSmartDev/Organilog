...
namespace Organilog.ViewModels.Interventions
{
    ...
    {
        private bool isDeniedLocation = false;
        ...
        private void GetInterventions(object sender)
        {
            ...
            }, token).ContinueWith(task => Device.BeginInvokeOnMainThread(() =>
            {
                if (task.Status == TaskStatus.RanToCompletion)
                {
                    ...
                    //ListIntervention = ListIntervention.OrderByDescending(i => i.DoneDateStart).ThenByDescending(i => i.DoneDateEnd).ThenBy(i => i.AddDate).ThenBy(i => i.EditDate).ThenByDescending(i => i.DoneHourStart).ThenByDescending(i => i.DoneHourEnd).ToList();
                    //ListIntervention = ListIntervention.OrderByDescending(i => i.AddDate).ToList();
                }
