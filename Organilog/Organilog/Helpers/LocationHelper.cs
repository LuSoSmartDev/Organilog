...
namespace Organilog.Helpers
{
    public class LocationHelper
    {
        ...
        public static async Task StartListening(TimeSpan? interval = null, double minimumDistance = 100)
        {
            ...
            await CrossGeolocator.Current.StartListeningAsync(interval ?? TimeSpan.FromMinutes(10), minimumDistance, true);
        ...
        private static void OnPositionChanged(object sender, PositionEventArgs e)
        {
            ...
            Location sourceCoordinates = new Location(Settings.LogLat, Settings.LogLong); // last of position 
            Location destinationCoordinates = new Location(position.Latitude, position.Longitude);// new position 

            double distance = Location.CalculateDistance(sourceCoordinates, destinationCoordinates, DistanceUnits.Kilometers);
            if (distance > 0.1) { //if distance > 100m 
                    ...
                    Settings.LogLat = position.Latitude;  //new pos 
                    Settings.LogLong = position.Longitude; //new pos 
